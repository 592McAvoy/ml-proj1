...
class KernelLogisticRegression(BaseModel):
    def __init__(self, N, kernel='rbf', loss='logistic', lamb=0.1, reg='lasso'):
        super().__init__()
        self.lamb = lamb
        self.loss_type = loss
        self.reg_type = reg
    ...
    def fit(self, x, y):
        ...
        if self.loss_type == 'logistic':
            ...
            loss_cls /= y.size(0)
        elif self.loss_type == 'least_square':
            loss_cls = F.mse_loss(h.squeeze(), y.float())
        ...
        return h, (loss, loss_cls, loss_reg)
