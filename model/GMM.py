...
class GaussianMixtureModel(BaseModel):
    def __init__(self, n_fea=256, n_class=10, device='cuda:0'):
        super().__init__()
        self.n_fea = n_fea  # D
        self.n_class = n_class  # K

        self.mu = nn.Parameter(torch.Tensor(
            1, n_class, n_fea), requires_grad=False).to(device)  # (1, K, D)
        # (1, K, D)
        self.var = nn.Parameter(torch.Tensor(
            1, n_class, n_fea), requires_grad=False).to(device)
        self.pi = nn.Parameter(torch.Tensor(
            1, n_class, 1), requires_grad=False).to(device)  # (1, K, 1)
    ...
    def _cal_log_p(self, x):
        ...
        # print(mu.device, self.mu.device)
        ...
        log_p = -0.5*(term0+term1+term2).to(x.device)  # N, K, 1
